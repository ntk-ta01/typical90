...
fn main() {
    ...
    let mut lazy_seg: LazySegtree<MaxUpdate> = vec![0; w + 1].into();
    let mut ans = vec![];
    for (l, r) in query {
        let h = lazy_seg.prod(l, r + 1);
        ans.push(h + 1);
        lazy_seg.apply_range(l, r + 1, h + 1);
    }
