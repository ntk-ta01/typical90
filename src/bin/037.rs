...
fn main() {
    ...
    // dp[i][j] := i番目までの香辛料を合計でj[mg]使うときの価値の最大値

    let mut dp = vec![];
    for _ in 0..n + 1 {
        let seg = Segtree::<Max<usize>>::new(w + 1);
        dp.push(seg);
    }
    for i in 0..n {
        for j in 0..=w {
            // dp[i][(j - R).max(0)..=(j - L).max(0)]のmaxを高速に取得したい
            let r = if j < data[i].0 { 0 } else { j - data[i].0 + 1 };
            let l = if j < data[i].1 { 0 } else { j - data[i].1 };
            let prod = dp[i].prod(l, r);
            let x = if r == 1 || r > 1 && prod != 0 {
                dp[i].get(j).max(prod + data[i].2)
            } else {
                dp[i].get(j)
            };
            dp[i + 1].set(j, x);
...
pub use self::internal_bit::*;
pub use self::internal_type_traits::*;
